...
async function robot() {
	...
	async function convertImage(sentenceIndex) {
		return new Promise((resolve, reject) => {
			const inputFile = fromRoot(
				`./content/${sentenceIndex}-original.png[0]`
			);
			const outputFile = fromRoot(
			...
			gm()
				...
				.write(outputFile, (error) => {
					...
					console.log(
						`> [video-robot] Image converted: ${outputFile}`
					);
					resolve();
				});
		});
	...
	async function createAllSentenceImages(content) {
		if (os.platform != "darwin" || os.platform != "win32") return;
		...
		return new Promise((resolve, reject) => {
			...
			gm()
				...
				.write(outputFile, (error) => {
					...
					console.log(
						`> [video-robot] Sentence created: ${outputFile}`
					);
					resolve();
				});
		});
